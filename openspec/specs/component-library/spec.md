...
The system SHALL provide a `Button` component with variants: `primary`, `secondary`, `ghost`, `danger`. It MUST support `size` prop (`sm`, `md`, `lg`), `disabled` state, and `loading` state (with spinner). All variants MUST work in both light and dark modes.
...
- **THEN** skeleton rows are displayed matching the column count
