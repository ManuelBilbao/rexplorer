...
The system SHALL provide a `TimeAgo` component that displays a relative timestamp (e.g., "2 min ago") with the absolute datetime available on hover/tooltip. It MUST update periodically without page reload.
...
The system SHALL provide a `StatusBadge` component that renders transaction status as a colored badge: green "Success" for `status: true`, red "Failed" for `status: false`, gray "Pending" for `status: null`.
...
The system SHALL provide a `ChainBadge` component that displays a chain's name with a colored indicator. Each chain MUST have a consistent color.

#### Scenario: Chain badge
- **WHEN** `<ChainBadge chain="ethereum" />` is rendered
- **THEN** an "Ethereum" badge with the Ethereum-associated color is displayed
