  ...
  operation(:index,
    ...
    parameters: [
      ...
      before_index: [
        in: :query,
        type: :integer,
        description: "Transaction index cursor (within block)"
      ],
  ...
  defp parse_tx_pagination(params) do
    ...
    opts =
      if params["block_number"],
        do: [{:block_number, parse_int(params["block_number"])} | opts],
        else: opts

    opts =
      if params["before_block"],
        do: [{:before_block, parse_int(params["before_block"])} | opts],
        else: opts

    opts =
      if params["before_index"],
        do: [{:before_index, parse_int(params["before_index"])} | opts],
        else: opts
